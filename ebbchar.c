...
extern void transform(int,char*,int,char*);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   transform(sizeTransformationVector,transformationsVector,sizeMessageToSend,messageToSend);
   int error_count = 0;
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   if(actualMessage==1){
       ...
       sizeTransformationVector = (len / 4);
       int* bufferAsInts = (int*)buffer;
       int i = 0;
       for(;i<sizeTransformationVector;i++)
       {
           transformationsVector[i] = bufferAsInts[i];
       }
       actualMessage = 0;
   }
   return len;
